...
function applySettings(settings) {
	...
	setMirroredValue(elements.slotIndexRange, elements.slotIndexNumber, clampNumber(settings.slotIndex, 0, 7, 0));
	setMirroredValue(elements.stepSizeRange, elements.stepSizeNumber, clampNumber(settings.stepSize, 1, 25, 5));
}

function bindMirroredInputs(rangeInput, numberInput, min, max) {
	rangeInput.addEventListener("input", () => {
		setMirroredValue(rangeInput, numberInput, clampNumber(rangeInput.value, min, max, min));
		persistSettings();
	});

	numberInput.addEventListener("change", () => {
		setMirroredValue(rangeInput, numberInput, clampNumber(numberInput.value, min, max, min));
		persistSettings();
	});
}
...
function handlePluginMessage(payload) {
	...
	if (payload.sessions.length === 0) {
		...
		empty.textContent = payload.error ? "Preview unavailable." : "No sessions match this filter.";
